...
class TLSMsg:
  ...
  def to_record_layer_bytes( self, content_type=None, content=None ):
    ...
    return tls.TLSPlaintext.build( self.to_record_layer_struct( \
...
class TLSByteStreamParser:
  ...
  def fragment_reassembly( self, plain_text:TLSMsg=None )-> TLSMsg:
    ...
    fragment MAY be a full handshake message in which case
    the fragment is returned as the message. The TLSPlaintext
    structure considers that a full message is sent
    ...
    if plain_text.content_type == 'handshake' :
      ...
      if tls_message_bytes is not  None :
    ...
    if self.debug is not None and tls_message is not None:
      if self.debug.trace is True:
        self.debug.trace_bin( "handshake_message:", tls_message_bytes )
  ...
  def parse_single_msg( self )-> dict:
    ...
    if self.fragment != b'':
      ...
      if tls_msg.content is not None:
        return tls_msg
  ...
  def init_from_test_vector( self,  lurk_client=None, tls_handshake=None, ks=None):
    ...
    if self.tls_handshake.is_ks_proposed( ) is True :
      ...
      if self.conf[ 'tls13' ][ 'ephemeral_method' ] == 'cs_generated' :
        ...
        self.tls_handshake.msg_list[ -1 ][ 'data' ][ 'extensions' ]\
                [ key_share_index ][ 'extension_data' ]\
...
#        self.c_init_client_hello_update( lurk_resp  )
        ...
        self.tls_handshake.update_random( \
                pylurk.tls13.lurk_tls13.Freshness( self.lurk_client.freshness ) )
...
##    if self.conf[ 'ecdhe_authentication' ] is True:
    ...
    if self.c_init_client_hello is True:
      if has_proposed_psk_in_cs is True :
        ...
        secret_request = []
      lurk_resp = lurk_client.resp( 'c_init_client_hello', \
              handshake=[ self.content ], \
        ...
        for binder_key in binder_key_list:
          ...
          if self.debug is not None:
            if self.debug.trace is True:
              ...
              self.debug.handle_bin( f"binder_finished_key ({index})", \
                      ks.finished_key(  role='binder') )
...
class ExtKeyShare:
  ...
  def __init__( self, tls_client_conf, debug  ):
    ...
    ## when generated by the CS, the ke_entries are empty
      ...
      raise pylurk.lurk.lurk_lurk.ConfigurationError( \
              f"unexpected ephemeral_method {self.conf[ 'ephemeral_method' ]} ")
  ...
  def handle_server_hello( self, lurk_client, client_hello, tls_handshake, ks, tls_msg ) :
    ...
    if self.tls_mode in [ 'psk_ecdhe', 'psk' ]:
      ...
      ks = pylurk.tls13.lurk_tls13.KeyScheduler( \
              tls_hash=tls_handshake.get_tls_hash() , debug=tls_handshake.debug)
...
#    print( f"--- handshake_msg_list {handshake_msg_list}" )
  ...
  def init_content_from_lurk_resp( self, lurk_resp ):
    try:
      ...
      raise pylurk.lurk.lurk_lurk.ConfigurationError( \
              f"Cannot find sig_scheme in conf: {self.conf}" )
    self.content = {
