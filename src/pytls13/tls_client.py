...
#      'extensions' : [] } }
  ...
  def handle_server_hello( self, clt_conf, lurk_client, client_hello, tls_handshake, ks, tls_msg, engine_psk_db ) :
    ...
    self.c_register_tickets = self.get_c_register_tickets_status( engine_psk_db, tls_handshake )
    self.c_server_hello = False
...
##    sh.c_server_hello_update( lurk_resp, tls_handshake, ks )
  ...
  def get_c_register_tickets_status( self, engine_psk_db, tls_handshake ):
    ...
    ## r can be generated and PSK can be generated by the engine.
      ...
      if ( engine_psk_db.get_psk( selected_ticket ) is None or\
...
class EnginePSKDB:
  ...
  def add( self, conf, new_session_ticket, ks, tls_handshake ) :
    psk = None
    if ks.secrets[ 'r' ] is not None:
      psk = ks.compute_psk( ticket[ 'ticket_nonce' ] )
    ticket_info = { 'ticket' : new_session_ticket,
                    'psk' : psk,
  ...
  def get_ticket_info_list( self, conf ):
