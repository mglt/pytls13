...
class ClientTLS13Session:
  ...
  def __init__( self, clt_conf, engine_ticket_db=None, cs=None ) :
    ...
    self.lurk_client = pylurk.lurk_client.get_lurk_client_instance( \
            self.clt_conf[ 'lurk_client' ], cs=cs )
  ...
  def connect( self, ip=None, port=443 ):
    ...
    if self.debug is not None and self.debug.test_vector is True:
      ch.init_from_test_vector( lurk_client=self.lurk_client, \
              tls_handshake=self.tls_handshake, ks=self.ks )
    else:
      ch.init( lurk_client=self.lurk_client, \
              tls_handshake=self.tls_handshake, ks=self.ks,\
              engine_ticket_db=self.engine_ticket_db )
    ...
    self.stream_parser = pytls13.tls_client_handler.TLSByteStreamParser( \
            self.s, debug=self.debug, sender='server')
    while True:
      ...
      if tls_msg.content_type == 'handshake':
        if tls_msg.content[ 'msg_type' ] == 'server_hello' :
          ...
          self.ks = sh.handle_server_hello( self.lurk_client, ch, \
                  self.tls_handshake, self.ks, tls_msg )
          ...
          s_h_cipher = pylurk.tls13.crypto_suites.CipherSuite( \
                  cipher_suite, self.ks.secrets[ 'h_s' ] )
          if self.debug is not None:
          ...
          pylurk.debug.print_bin( "built certificate_request", \
                  pytls13.struct_tls13.Handshake.build( tls_msg.content ) )
        elif tls_msg.content[ 'msg_type' ] == 'certificate':
          certificate = pytls13.tls_client_handler.Certificate( \
                  conf=self.clt_conf, content=tls_msg.content, sender='server' )
          ...
          certificate_verify = pytls13.tls_client_handler.CertificateVerify(\
                  conf=self.clt_conf, content=tls_msg.content, sender='server' )
          ...
          sf = pytls13.tls_client_handler.Finished( conf=self.clt_conf,\
                  content=tls_msg.content, sender='server' )
    ...
    if self.c_init_client_hello is True:
      ...
      if self.tls_handshake.is_certificate_request_state is True:
        ...
        client_cert = pytls13.tls_client_handler.Certificate( \
                conf=self.clt_conf, content={}, sender='client' )
        client_cert.init_from_conf( )
        client_cert.encrypt_and_send( cipher=c_h_cipher, socket=self.s, \
                sender='client', debug=self.debug )
        ...
        client_cert_verify = pytls13.tls_client_handler.CertificateVerify(\
                conf=self.clt_conf, sender='client' )
        client_cert_verify.handle_c_client_finished( self.lurk_client, \
                self.ks, tmp_handshake, self.c_register_tickets )
        self.tls_handshake.msg_list.append(  client_cert_verify.content )
        client_cert_verify.encrypt_and_send( cipher=c_h_cipher, \
                socket=self.s, sender='client', debug=self.debug )
...
#        print( f":: C -> S: Sending {client_cert.content[ 'msg_type' ]}\n" )
        client_cert.encrypt_and_send( cipher=c_h_cipher, \
                socket=self.s, sender='client', debug=self.debug )
        self.tls_handshake.msg_list.append( client_cert.content )
        tmp_handshake.append( client_cert.content )
#        print( f"--- client certificate: {client_cert.content}" )
        client_cert_verify = pytls13.tls_client_handler.CertificateVerify( \
                conf=self.clt_conf, sender='client' )
        client_cert_verify.handle_c_init_client_finished( \
                self.lurk_client, self.ks, tmp_handshake, \
                self.c_register_tickets )
        self.tls_handshake.msg_list.append(  client_cert_verify.content )
#        print( f":: C -> S: Sending {client_cert_verify.content[ 'msg_type' ]}\n" )
    ...
    client_finished = pytls13.tls_client_handler.Finished( \
            conf=self.clt_conf, content=self.tls_handshake.msg_list[ -1 ], sender='client' )
    client_finished.encrypt_and_send( cipher=c_h_cipher,\
            socket=self.s, sender='client', debug=self.debug )
    self.s_a_cipher = pylurk.tls13.crypto_suites.CipherSuite( \
            cipher_suite, self.ks.secrets[ 'a_s' ] )
    self.stream_parser.cipher = self.s_a_cipher
    if self.debug is not None:
      self.s_a_cipher.debug( self.debug, description='server_application' )
    self.c_a_cipher = pylurk.tls13.crypto_suites.CipherSuite( \
            cipher_suite, self.ks.secrets[ 'a_c' ] )
    if self.debug is not None:
      self.c_a_cipher.debug( self.debug, description='client_application' )

  def send( self, data ):

    app_data = pytls13.tls_client_handler.TLSMsg( conf=self.clt_conf,\
            content=data, content_type='application_data', sender='client' )
    app_data.encrypt_and_send( cipher=self.c_a_cipher, \
            socket=self.s, sender='client', debug=self.debug )
...
#        return tls_msg.content
        ...
        if tls_msg.content[ 'msg_type' ] == 'new_session_ticket':
          new_session_ticket = pytls13.tls_client_handler.NewSessionTicket(\
                  conf=self.clt_conf, content=tls_msg.content )
