...
TEE is handled by `Gramine <https://github.com/gramineproject/>`_ a libOS framework that enable to run an application in a TEE.
...
``pylurk`` can be instantiated in various mode that involves parameters defined **at the system level** (i.e. how the python interpreter is started) or **at the application layer** (i.e. by the CS module itself).
...
#. ``gramine-manifest``\ : Generates a complete Manifest (which can be viewed as building a complete configuration)
#. ``gramine-sgx-sign``\ : generate the SIGSTSTRUCT, the measurements and signs it. ( in a ``.sig`` file).
   In addition, all included files are hashed in order to ensure they have not changed.
#. ``gramine-get-token``\ : Generates a token that has the same format as the EINITOKEN.
...
This section is based on the `Secure Processors Part I: Intel SGX Security Analysis and MIT Sanctum Architecture <https://people.csail.mit.edu/devadas/pubs/part_1.pdf>`_.
...
.. image:: ./fig/signature_structure.png
   :target: ./fig/signature_structure.png
   :alt: 
...
The idea is to load a software and that **attestation** provides some evidence that the software is what we expect.
...
The ``load_enclave`` (in `host_main.c <https://github.com/gramineproject/gramine/blob/38aa937772c8f6d74d94f7d84dddf2cd3e2c2c54/pal/src/host/linux-sgx/host_main.c>`_ ) proceeds to the initialization of the enclave.
